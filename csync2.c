...
char *update_basis_filename = NULL;
char *update_new_filename = NULL;
char *update_new_filename_buffer = NULL;
char *partition_buffer = NULL;
...
"	-O	List a count of all dirty files from status db\n"
"\n"
...
"	-Z	Remove dirty entries when doing a -TI run.\n"
"\n"
...
char *paranoid_tmpdir_name(char *name)
{
	if ( access(P_tmpdir, R_OK|W_OK|X_OK) < 0 )
		csync_fatal("Temp directory '%s' does not exist!\n", P_tmpdir);

	if ( !(mkdtemp(name)) )
		csync_fatal("ERROR: mkdtemp() didn't return a valid directory name!\n");

	return name;
}

char *paranoid_tmpfile_just_name(char *name)
{
	if ( access(P_tmpdir, R_OK|W_OK|X_OK) < 0 )
		csync_fatal("Temp directory '%s' does not exist!\n", P_tmpdir);

	if ( !(name = mktemp(name)) )
		csync_fatal("ERROR: mktemp() didn't return a valid file name!\n");
...
FILE *paranoid_tmpfile_name(char *name)
{
	...
	if ( !(fd = mkstemp(name)) )
		csync_fatal("ERROR: mkstemp() didn't return a valid file descriptor!\n");

	if (!( f = fdopen(fd, "wb")))
		csync_fatal("ERROR: fopen-mktemp() didn't return a valid file handle!\n");
...
int main(int argc, char ** argv)
{
	...
	update_new_filename_buffer = (char*) malloc(32*sizeof(char));
	partition_buffer = (char*) malloc(32*sizeof(char));
	...
	while ( (opt = getopt(argc, argv, "W:s:Ftp:G:P:C:D:N:HBAIXZULSTMORvhcuoimfxrd")) != -1 ) {
		switch (opt) {
			case 'W':
	...
	signal(SIGTSTP, csync_db_pause);
	signal(SIGCONT, csync_db_continue);
	gen_dblock_timeout();
...
found_a_group:;
	...
	switch (mode) {
		...
		case MODE_TEST_SYNC: {
			...
			if (init_run && init_run_straight)
				init_run |= 4;
			if (init_run && init_run_remove_dirty)
				init_run |= 8;
			switch (argc-optind)
			{
			case 3:
		...
		case MODE_LIST_DIRTY_COUNT:
			retval = 0;
			SQL_BEGIN("DB Dump - Dirty Count",
	...
	if ( csync_error_count != 0 || (csync_messages_printed && csync_debug_level) )
		csync_debug(0, "Finished with %d errors and %d conflicts.\n", csync_error_count,csync_conflict_count);

	csync_printtotaltime();

	if ( retval >= 0 && csync_error_count == 0 ) return retval;
	return (csync_error_count != 0 && csync_error_count > csync_conflict_count);
}
