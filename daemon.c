...
{
	update_new_filename = update_new_filename_buffer;
	if (strncmp(name,"/var/aegir/",11) == 0) {
		strcpy(update_new_filename,"/var/aegir/tmp/csync2_tmp.XXXXXX");
	} else if (strncmp(name,"/var/",5) == 0) {
		strcpy(update_new_filename,"/var/tmp/csync2_tmp.XXXXXX");
	} else if (strncmp(name,"/etc/",5) == 0) {
		strcpy(update_new_filename,"/etc/tmp/csync2_tmp.XXXXXX");
	} else {
		strcpy(update_new_filename,"/tmp/csync2_tmp.XXXXXX");
	}
	csync_debug(3, "Generated tmp name: %s based on name: %s\n",
		update_new_filename,name);
}
...
enum {
	...
	A_DEBUG, A_HELLO, A_BYE, A_DONE, A_SIGDEL, A_SETMET
};

struct csync_command cmdtab[] = {
	{ "sig",	1, 0, 0, 0, 1, A_SIG	},
	{ "sigdel",	1, 0, 0, 0, 1, A_SIGDEL	},
...
void csync_daemon_session()
{
	...
	int i;
	int do_setown, do_setmod, do_setime;
	...
	while ( conn_gets(line, 4096) ) {
		...
		case A_MKDIR:
			{
				...
				if ( lstat_strict(prefixsubst(tag[2]), &st) != 0 || !S_ISDIR(st.st_mode)) {
					...
					//sets update_new_filename
					set_update_new_filename_on_same_partition(prefixsubst(tag[2]));
					paranoid_tmpdir_name(update_new_filename);
					update_basis_filename = prefixsubst(tag[2]);

					if ( mkdir(update_new_filename, 0700) ) {
						struct stat st;
						if ( lstat_strict(update_new_filename, &st) != 0 || !S_ISDIR(st.st_mode))
							cmd_error = strerror(errno);
					}
				} else {
		...
		case A_MKLINK:
			//sets update_new_filename
			set_update_new_filename_on_same_partition(prefixsubst(tag[2]));
			paranoid_tmpfile_just_name(update_new_filename);
			update_basis_filename = prefixsubst(tag[2]);
			if ( symlink(tag[3], update_new_filename) )
				cmd_error = strerror(errno);
			break;
		case A_MKSOCK:
		...
		case A_SETMET:
			set_meta_data=1;
			tag_offset=3;
			do_setown=atoi(tag[3]);
			do_setmod=atoi(tag[4]);
			do_setime=atoi(tag[5]);
		case A_SETOWN:
			if (( !csync_ignore_uid || !csync_ignore_gid ) && (!set_meta_data || do_setown)) {
				int uid = csync_ignore_uid ? -1 : name_to_uid(tag[3+tag_offset]);
				int gid = csync_ignore_gid ? -1 : group_to_gid(tag[4+tag_offset]);
				if ( ! update_new_filename ) {
					csync_debug(3, "Performing SETOWN with %d:%d on %s\n",uid,gid,prefixsubst(tag[2]));
					if ( lchown(prefixsubst(tag[2]), uid, gid) )
						cmd_error = strerror(errno);
				} else {
					csync_debug(3, "Performing SETOWN with %d:%d on %s\n",uid,gid,update_new_filename);
					if ( lchown(update_new_filename, uid, gid) )
						cmd_error = strerror(errno);
				}
			}
			if (set_meta_data  && !cmd_error)
				tag_offset+=2;
			else
				break;
		case A_SETMOD:
			if ( !csync_ignore_mod && (!set_meta_data || do_setmod)) {
				if ( ! update_new_filename ) {
					csync_debug(3, "Performing SETMOD with %d on %s\n",atoi(tag[3+tag_offset]),prefixsubst(tag[2]));
					if ( chmod(prefixsubst(tag[2]), atoi(tag[3+tag_offset])) )
						cmd_error = strerror(errno);
				} else {
					csync_debug(3, "Performing SETMOD with %d on %s\n",atoi(tag[3+tag_offset]),update_new_filename);
					if ( chmod(update_new_filename, atoi(tag[3+tag_offset])) )
						cmd_error = strerror(errno);
				}
			}
			if (set_meta_data && !cmd_error)
		...
		case A_SETIME:
			{
				if (!set_meta_data || do_setime) {
					struct utimbuf utb;
					utb.actime = atoll(tag[3+tag_offset]);
					utb.modtime = atoll(tag[3+tag_offset]);

					if ( ! update_new_filename ) {
						csync_debug(3, "Performing SETIME with %Ld on %s\n",utb.actime,prefixsubst(tag[2]));
						if ( utime(prefixsubst(tag[2]), &utb) )
							cmd_error = strerror(errno);
					} else {
						csync_debug(3, "Performing SETIME with %Ld on %s\n",utb.actime,update_new_filename);
						if ( utime(update_new_filename, &utb) )
							cmd_error = strerror(errno);
					}
				}
			}
		...
		case A_DONE:
			{
				if (! update_new_filename && ! update_basis_filename) {
					csync_debug(3, "New filename and Basis filename were NULL\n");
				} else if ( ! update_new_filename ) {
					csync_debug(3, "New filename was NULL\n");
				} else if ( ! update_basis_filename ) {
					csync_debug(3, "Basis filename was NULL\n");
				} else {
					csync_debug(3, "Atomically moving %s to %s\n",update_new_filename,update_basis_filename);
					if (rename(update_new_filename,update_basis_filename))
						cmd_error = strerror(errno);
				}
				update_new_filename = NULL;
				update_basis_filename = NULL;
			}
			break;
		case A_LIST:
...
found_asactive: ;
		...
		if ( cmdtab[cmdnr].update == 1 ) {
			if ( cmdtab[cmdnr].action == A_DEL )
				csync_debug(1, "Deleted %s from %s.\n",
					tag[2], peer ? peer : "???");
			else
...
abort_cmd:
		if ( cmd_error ) {
			conn_printf("%s\n", cmd_error);
			//Delete any temp file and clear the pointers so that we do not keep
			//this filename around for the next file's commands
			if (update_new_filename) {
				struct stat st;
				lstat_strict(update_new_filename, &st);
				S_ISDIR(st.st_mode) ? rmdir(update_new_filename) : unlink(update_new_filename);
			}
			update_new_filename = NULL;
			update_basis_filename = NULL;
		}
		else if (cmdtab[cmdnr].action  != A_SIGDEL)
