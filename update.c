...
int read_conn_status(const char *file, const char *host)
{
	...
	csync_error_count++;
	int rc = !strcmp(line, "File is also marked dirty here!") ? 1 : 2;
	if (rc == 2) csync_conflict_count++;
	return rc;
}
...
{
	struct stat st;
	int last_conn_status = 0, auto_resolve_run = 0, making_change = 0, skipping_first_action = 0;
	const char *key = csync_key(peername, filename);
	const char *chk2;
	char chk1[4096];
	int uid_diff, gid_diff, mode_diff, mtime_diff;
...
auto_resolve_entry_point:
	...
	if ( force ) {
		...
		if (!rs_check_result) {			
			if (S_ISREG(st.st_mode)) {
				csync_debug(1, "File content is already up to date on peer. Setting meta-data.\n"); 
				skipping_first_action=1;
			} else {
				csync_debug(1, "File content is already up to date on peer. Non regular file so acting as normal.\n"); 			
			}		
		}
		if ( dry_run ) {
			printf("?M: %-15s %s\n", peername, filename);
	...
	if (skipping_first_action) {
		goto skip_first_action;
	}
...
skip_first_action:
	uid_diff=!csync_checkvalue(chk1,chk2,"uid");
	gid_diff=!csync_checkvalue(chk1,chk2,"gid");
	mode_diff=!S_ISLNK(st.st_mode) && (!csync_checkvalue(chk1,chk2,"mode") || making_change);
	mtime_diff=!S_ISLNK(st.st_mode) && (!csync_checkvalue(chk1,chk2,"mtime") || making_change);

	if (uid_diff || gid_diff || mode_diff || mtime_diff || making_change) {
		//Shortcut the round trip times and send a single message to set all metadata
		conn_printf("SETMET %s %s %d %d %d %s %s %d %Ld\n",
				url_encode(key), url_encode(filename),
				uid_diff | gid_diff | making_change, mode_diff, mtime_diff, uid_to_name(st.st_uid), gid_to_group(st.st_gid), st.st_mode, (long long)st.st_mtime);
...
skip_action:
	if ( making_change && (S_ISLNK(st.st_mode) || S_ISREG(st.st_mode) || S_ISDIR(st.st_mode))) {
		conn_printf("DONE %s %s \n",
			url_encode(key), url_encode(filename));
		if ( read_conn_status(filename, peername) )
			goto got_error;
	}
			...
			case CSYNC_AUTO_METHOD_SMALLER:
				{
					...
					//Use >= not just >. This way we might sometime copy a file that we don't need to
					//but it stops us getting stuck in a losing loop for the conflict.
				...
				if ( rel < 0 ) {
					...
					if ( !rel ) {
						if ( strcmp(l_checktxt, r_checktxt) ) {
							...
							if (init_run & 8) {
								printf("S\t%s\t%s\t%s\n", myname, peername, l_file); ret=0;
								SQL("Remove matching file from dirty db",
			    				"DELETE FROM dirty WHERE filename = '%s' AND peername = '%s'", l_file, peername);
		    				}
						}
					}
				}
