...
int csync_recv_file(FILE *out)
{
	...
	if ( !conn_gets(buffer, 100) || sscanf(buffer, "octet-stream %ld\n", &size) != 1 ) {
		...
		csync_fatal("Format-error while receiving data (%s).\n",buffer);
	}

	csync_debug(4, "Receiving %ld bytes ..\n", size);
	...
	while ( size > 0 ) {
		...
		size -= chunk;
		csync_debug(4, "Got %d bytes, %ld bytes left ..\n",
				chunk, size);
	}
...
int csync_rs_patch(const char *filename)
{
	...
	update_basis_filename = prefixsubst(filename);
	basis_file = fopen(update_basis_filename, "rb");
	if ( !basis_file ) basis_file = paranoid_tmpfile();
	...
	set_update_new_filename_on_same_partition(filename);
	new_file = paranoid_tmpfile_name(update_new_filename);
	...
	csync_debug(3, "Remembered to move new data from temp file %s to local file %s ..\n", update_new_filename, update_basis_filename);
/*
...
/*
	...
	while ( (rc = fread(buffer, 1, 16384, new_file)) > 0 )
